...
async function syncLocalProfile(user, localPhotoOverride) {
  ...
  const profilePicture = localPhoto || cachedPhoto || existingPhoto || remotePhoto;
...
export function wireProfileEvents(onRender) {
  ...
  const saveAppearanceBtn = document.getElementById("save-account-appearance");
  if (saveAppearanceBtn) saveAppearanceBtn.addEventListener("click", async () => {
    const themeInput = document.getElementById("account-theme");
    const darkInput = document.getElementById("account-dark");
    const message = document.getElementById("account-appearance-message");
    const user = getCurrentUser();

    if (!user || !user.uid) {
      if (message) {
        message.style.color = "var(--danger)";
        message.innerText = "You must be signed in to update appearance.";
      }
      return;
    }

    const state = getState();
    const theme = String((themeInput && themeInput.value) || state.prefs.theme || "teal");
    const dark = String((darkInput && darkInput.value) || String(Boolean(state.prefs.dark_mode))) === "true";

    try {
      if (message) {
        message.style.color = "var(--text-soft)";
        message.innerText = "Saving appearance...";
      }
      ...
      await updateUserAccountPrefs(user.uid, {
        theme,
        dark_mode: dark,
        profile_picture: String(next.prefs.profile_picture || ""),
      });

      trackProfileLog("account_appearance_updated", "Updated account appearance preferences", {
        theme,
        dark_mode: dark,
      });
      ...
      if (message) {
        ...
        message.innerText = "Appearance updated for all inventory spaces.";
      }
